...
while (E > 1e-3) && (Itr < 150)
    ...
    for ii = 1:numel(Ids)
        ...
        dq = dq + 0.1*lam1*J.'*((J*J.' + lam2*eye(6))\Fu);
    end
...
Dmod = Shapes.Material.getContactDamping();
...
Id = Shapes.Contact.intersect(Pos,Shapes.TubeRadiusA); 
Set = 1:Shapes.NNode;
DetectionSet = Set(Id);
...
for ii = DetectionSet
    ...
    if ~isempty(ConXY)
        [T_,N_,B_] = Shapes.Contact.normal(g(1:3,4,ii).');
    ...
    if abs(norm(Rw))>0
       alpha = clamp(dot(-VT,N_),0,1);
    else
       alpha = 0;
    end

    % project wrench onto conf. space
    Fr = Fr + JJ.'*(Rw + Rf) - alpha*Dmod*JJ.'*(Vt);
...
function [Kc, Dc] = computeContactJacobian(Shapes,Q0,dQ0)
